...
void PWM_channel_init() {
    // enable input capture output compare 1 (initial in output mode)
    SET_REG(TIM2->CCER, TIM_CCER_CC1E, TIM_CCER_CC1E);
    // set output compare to PWM mode 1 (active if CCR < CNT)
    SET_REG(TIM2->CCMR1, TIM_CCMR1_OC1M, TIM_CCMR1_OC1M_0 * 6);
}
...
void GPIO_init() {
    ...
    SET_REG(GPIOC->MODER, GPIO_MODER_MODER13, 0);
    SET_REG(GPIOC->PUPDR, GPIO_PUPDR_PUPD13, GPIO_PUPDR_PUPD13_1);

    // Buzzer
    SET_REG(GPIOA->MODER, GPIO_MODER_MODE5, GPIO_MODER_MODE5_1);
    SET_REG(GPIOA->AFR[0], GPIO_AFRL_AFRL5, GPIO_AFRL_AFSEL5_0);
}
...
void EXTI_config() {
    ...
    SET_REG(SYSCFG->EXTICR[1], SYSCFG_EXTICR2_EXTI4, SYSCFG_EXTICR2_EXTI4_PA);
    SET_REG(SYSCFG->EXTICR[1], SYSCFG_EXTICR2_EXTI6, SYSCFG_EXTICR2_EXTI6_PA);
    SET_REG(SYSCFG->EXTICR[1], SYSCFG_EXTICR2_EXTI7, SYSCFG_EXTICR2_EXTI7_PA);
    SET_REG(SYSCFG->EXTICR[2], SYSCFG_EXTICR3_EXTI8, SYSCFG_EXTICR3_EXTI8_PA);
...
void NVIC_config() {
    ...
    NVIC->ISER[1] |= 0b1 << (EXTI15_10_IRQn - 32);
}

void EXTI4_IRQHandler() {
    if (GET_REG(EXTI->PR1, EXTI_PR1_PIF4)) {
        if (count_down < 0) count_down = keypad_scan(0) * 100;
...
void SysTick_Handler(void) {
    ...
    if (count_down == 0) {
        // beep
        SET_REG(TIM2->CR1, TIM_CR1_CEN, 0);
        // set arr for pressed frequency
        TIM2->ARR = 10000000 / 4153;
        // set PWM with duty cycle
        TIM2->CCR1 = 10000000 / 2616 * 50 / 100;
        // reset timer
        TIM2->EGR = TIM_EGR_UG;
        // start timer
