...
int main(int argc, char *argv[])
{
  ...
  #pragma omp task
  memset(q, 0, sizeof(double) * cols);
  #pragma omp task
  memset(z, 0, sizeof(double) * cols);
  #pragma omp task
  memset(r, 0, sizeof(double) * cols);
  #pragma omp task
  memset(p, 0, sizeof(double) * cols);
                      ...
                      double *rnorm)
{
  ...
  for (cgit = 1; cgit <= cgitmax; cgit++) {
    ...
    #pragma omp parallel for private(j,k,sum) schedule(guided, 32)
    for (j = 0; j < lastrow - firstrow + 1; j++) {
      sum = 0.0;
      for (k = rowstr[j]; k < rowstr[j+1]; k++) {
    ...
    //---------------------------------------------------------------------
    rho = 0.0;
    #pragma omp parallel for private(j) schedule(guided, 32) reduction(+:rho)
    for (j = 0; j < cols; j++) {
      z[j] = z[j] + alpha*p[j];  
      r[j] = r[j] - alpha*q[j];
      rho = rho + r[j]*r[j];
